...
class MyHashSet
    ...
    def union(set)
        ...
        set.to_a.each do |el|
            s.insert(el) unless s.include?el
        end
        s
    end
    ...
    def minus(set)
        ...
        self.to_a.each do |el|
            s.insert(el) unless set.include?(el)
        end
        s
    end
    ...
    def ==(object)
        return false unless object.instance_of?(MyHashSet)
        return false unless self.length == object.length

        object.to_a.each do |el|
